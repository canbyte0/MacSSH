#!/bin/bash
#
# MacSSH Phase 11 聚焦验证脚本（私钥路径，不触碰 Keychain 密码）
#
# 用途：
...
#   - 运行聚焦测试套件：TransferQueueRealTests（50 任务队列压力 /
#     失败隔离 / 20 轮取消运行中任务 / 多会话隔离 / 断线重连补位）、
#     TransferQueueTests（纯模型调度器单测）与
#     TransferResourceTests（资源基线-峰值-恢复）
#
# 前置：
...
TEST_LOG="/tmp/macssh-phase11-focus-tests.log"
TEST_BUILD_LOG="/tmp/macssh-phase11-focus-build.log"

PHASE6_KEY_NOPASS="/tmp/macssh_phase6_ed25519"
AUTHORIZED_KEYS="$HOME/.ssh/authorized_keys"
AK_BEGIN="# macssh-phase11-focus-begin"
AK_END="# macssh-phase11-focus-end"

PHASE10_FIXTURE_PATH_FILE="/tmp/macssh_phase10_fixture_path"
PHASE11_FIXTURE_ROOT="/tmp/macssh-phase11-$(uuidgen | tr '[:upper:]' '[:lower:]')"
...
mkdir -p "$HOME/.ssh"
chmod 700 "$HOME/.ssh"
ssh-keygen -t ed25519 -f "$PHASE6_KEY_NOPASS" -N "" -C "macssh-phase11-focus" -q
...
echo "==> 运行聚焦测试（TransferQueueTests / TransferQueueRealTests / TransferResourceTests）"
status=0
xcodebuild test-without-building \
    ...
    -skipPackagePluginValidation \
    -skipMacroValidation \
    -only-testing:MacSSHTests/TransferQueueTests \
    -only-testing:MacSSHTests/TransferQueueRealTests \
    -only-testing:MacSSHTests/TransferResourceTests \
    > "$TEST_LOG" 2>&1 || status=$?
