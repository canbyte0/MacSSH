...
TRANSLATIONS: list[tuple[str, str, str]] = [
    ...
    ("agent.empty.subtitle", "Ask anything — the Agent can read the terminal context and files under the current directory. Commands run in a separate non-interactive process, never in the current interactive Terminal, and every command requires your explicit approval.", "随时提问——Agent 可读取终端上下文与当前目录中的文件。命令会在独立的非交互进程中执行，不会直接输入当前交互式 Terminal，并且每条命令都需要你的明确批准。"),
    ...
    ("agent.tool.run_command", "Run command", "执行命令"),
    ("agent.tool.unknown", "Tool call", "工具调用"),
    ("agent.tool.status.running", "Running", "进行中"),
    ("agent.tool.status.awaiting_approval", "Awaiting approval", "等待批准"),
    ("agent.tool.status.success", "Done", "完成"),
    ("agent.tool.status.failure", "Failed", "失败"),
    ("agent.tool.status.denied", "Denied", "已拒绝"),
    ("agent.tool.status.cancelled", "Cancelled", "已取消"),
    ("agent.tool.status.timed_out", "Timed out", "已超时"),
    ("agent.command.target", "Target", "目标"),
    ("agent.command.target.local", "Local", "本地"),
    ("agent.command.target.remote", "Remote SSH", "远程 SSH"),
    ("agent.command.working_directory", "Working directory", "工作目录"),
    ("agent.command.provider", "AI service", "AI 服务"),
    ("agent.command.command", "Exact command", "完整命令"),
    ("agent.command.disclosure", "If approved, bounded stdout and stderr will be returned to %@.", "批准后，有界的 stdout 和 stderr 将返回给 %@。"),
    ("agent.command.approve", "Approve", "批准"),
    ("agent.command.deny", "Deny", "拒绝"),
    ("agent.command.approve.hint", "Approve this command to start execution.", "批准后开始执行此命令。"),
    ("agent.command.deny.hint", "Reject this command before it starts.", "在命令开始前拒绝执行。"),
    ...
    ("agent.settings.api_key.help", "The API Key is stored in the macOS Keychain and never displayed back. The Agent can read the terminal context and files under the current directory. Commands run in a separate non-interactive process, never in the current interactive Terminal, and every command requires your explicit approval.", "API Key 保存在 macOS 钥匙串中，保存后不再回显。Agent 可读取终端上下文与当前目录中的文件。命令会在独立的非交互进程中执行，不会直接输入当前交互式 Terminal，并且每条命令都需要你的明确批准。"),
